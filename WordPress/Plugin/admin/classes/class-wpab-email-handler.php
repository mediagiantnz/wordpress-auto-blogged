...
class WPAB_Email_Handler {
    ...
    public function send_approval_email( $post_id, $topic ) {
        ...
        // Get recipient email - use unified notification email
        $recipient_email = isset( $options['notification_email_address'] ) ? $options['notification_email_address'] : get_option('admin_email');
        ...
        // Build approval URLs using simple query strings
        $approve_url = add_query_arg( array(
            'wpab_approval' => 'approve',
            'key' => $approve_key,
            'post_id' => $post_id
        ), home_url() );
        
        $reject_url = add_query_arg( array(
            'wpab_approval' => 'reject',
            'key' => $reject_key,
            'post_id' => $post_id
        ), home_url() );
        ...
        // Send email
        $this->send_email( $recipient_email, $subject, $html_content );
    }
    ...
    private function send_email( $to, $subject, $html_content, $text_content = '' ) {
        ...
        if ( $email_provider === 'mailgun' ) {
            return $this->send_mailgun_email( $to, $subject, $html_content, $text_content );
        } else {
...
<body>
    <div class="container">';
        ...
        <div class="content">
            ...
            <div class="notice">
                <strong>Note:</strong> You must be logged in to your WordPress admin account to approve, reject, or edit posts. 
