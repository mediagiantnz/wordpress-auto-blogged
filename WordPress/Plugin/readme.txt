...
= 2.0.10 =
* Fixed: Critical PHP parse error in class-wpab-public.php
* Fixed: Method declaration outside of class causing fatal error
* Fixed: Syntax error preventing plugin activation

= 2.0.9 =
* Fixed: LiteSpeed 403 errors by using frontend query strings instead of admin-post.php
* Fixed: Approval URLs now use simple home URL with query parameters
* Added: Frontend approval handler to bypass server security restrictions
* Improved: More compatible with security plugins and server configurations
* Added: Visual feedback on frontend after approval/rejection

= 2.0.8 =
* Fixed: Approval/reject URLs now use admin-post.php instead of custom rewrite rules
* Fixed: Eliminated 404 errors on approval/reject actions
* Improved: Simpler and more reliable approval URL handling
* Added: Success notices after approval/rejection in admin area
* Removed: Custom rewrite rules that were causing conflicts

= 2.0.7 =
* Fixed: Approval/reject/edit URLs now work correctly with authentication
* Fixed: jQuery not defined error by ensuring proper script dependencies
* Added: Login requirement notice in approval emails
* Fixed: Email button text color for better contrast (white text)
* Improved: Approval URLs now include post ID for better validation
* Improved: Rewrite rules flushed on plugin setup
* Added: Redirect to posts list after approval/rejection
* Security: Approval actions require login and edit_posts capability

= 2.0.6 =
* Fixed: Posts now use configured default status instead of always draft
* Added: Email provider selection (WordPress default or Mailgun)
* Added: Email provider configuration in setup wizard
* Added: API key masking for security (shows only first 8 characters)
* Added: Default post status setting in Post Settings
* Improved: Consolidated notification emails to single address
* Improved: Email handler now supports both wp_mail and Mailgun
* Improved: Setup wizard now includes email configuration
* Fixed: Topics not being parsed correctly from Claude responses

= 2.0.5 =
* Fixed deprecated WP_User_Query 'who' parameter - now uses 'capability'
* Added error suppression for problematic plugins during activation
* Improved plugin initialization with delayed loading
...
= 2.0.10 =
CRITICAL: Fixes fatal PHP error. Update immediately if experiencing site errors.
